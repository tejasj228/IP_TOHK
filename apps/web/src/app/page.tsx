...
// Constants
const NOTIFICATION_SHOW_DELAY = 1000;
const NOTIFICATION_AUTO_HIDE_DELAY = 16_000;
...
const AnimatedCounter = ({
  ...
  useEffect(() => {
    ...
    const updateCount = () => {
      ...
      const easeOutQuart = 1 - (1 - progress) ** EASING_POWER;
...
const SummaryCard = ({
  ...
  >
    ...
    >
      <Icon
        className={`${compact ? "h-6 w-6" : "h-6 w-6 sm:h-8 sm:w-8"} transform text-[gold] transition-transform duration-800 ease-in-out group-hover:rotate-[360deg]`}
      />
    </div>
    <h4
      ...
      >
        ...
        <div className="absolute inset-0">
          {heroImages.map((image, index) => (
            <div
              ...
              key={`hero-bg-${index}-${image.split("/").pop()}`}
              style={{
        ...
        <div className="relative z-10 h-full px-4 sm:px-6 lg:px-12 xl:px-16">
          ...
          <button
            ...
            className="-translate-y-1/2 absolute top-1/2 left-4 z-30 hidden rounded-full bg-white/20 p-2 text-white backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-[gold] sm:left-6 sm:p-3 md:inline-flex lg:left-8"
          ...
          <button
            ...
            className="-translate-y-1/2 absolute top-1/2 right-4 z-30 hidden rounded-full bg-white/20 p-2 text-white backdrop-blur-sm transition-all duration-300 hover:scale-110 hover:bg-white/30 focus:outline-none focus:ring-2 focus:ring-[gold] sm:right-6 sm:p-3 md:inline-flex lg:right-8"
            onClick={goToNext}
            type="button"
          >
                ...
                <div className="flex items-center gap-2">
                  {heroImages.map((image, index) => (
                    <button
                      ...
                      key={`hero-mobile-dot-${index}-${image.split("/").pop()}`}
              ...
              <div className="-translate-x-1/2 absolute bottom-8 left-1/2 z-20 hidden transform space-x-2 lg:flex">
                {heroImages.map((image, index) => (
                  <button
                    ...
                    key={`hero-image-indicator-${index}-${image.split("/").pop()}`}
                    onClick={() => setCurrentImageIndex(index)}
                    type="button"
                  />
                ))}
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-12 sm:mb-16">
              <div className="group relative aspect-video w-full overflow-hidden rounded-2xl bg-gray-900">
                ...
                <div className="absolute bottom-4 left-4 z-30 rounded-lg bg-[gold] px-3 py-2 opacity-100 shadow-lg transition-opacity duration-300 sm:bottom-6 sm:left-6">
                  <p className="font-bold text-[#050a30] text-xs sm:text-sm">
                    YCB Trailer
                  </p>
                </div>
              </div>
            ...
            <div className="mb-12 sm:mb-16">
              ...
              <div className="mx-auto hidden max-w-6xl grid-cols-2 items-stretch gap-6 md:grid lg:grid-cols-3 lg:gap-8">
                {summaryPoints.map(({ Icon, title, desc }) => (
                  <SummaryCard
                    desc={desc}
                    Icon={Icon}
                    key={`summary-grid-${title.replace(/\s+/g, "-").toLowerCase()}`}
                    title={title}
                  />
                ))}
              </div>
      ...
      >
        ...
        <div className="relative z-10">
          <div className="mx-auto max-w-7xl text-center">
            ...
            <div className="-ml-[50vw] -mr-[50vw] relative right-[50%] left-[50%] mt-4 mb-8 flex w-[100vw] justify-end pr-4 sm:pr-6 lg:pr-10">
              <TestimonialsControls />
            </div>

            {/* Testimonial Cards with Marquee */}
            <div className="-ml-[50vw] -mr-[50vw] relative right-[50%] left-[50%] w-[100vw]">
              <TestimonialsMarquee className="[--duration:30s]" />
            </div>
          </div>
        </div>
      </section>
      ...
      >
        {summaryPoints.map(({ Icon, title, desc }) => (
          <div
            ...
            key={`summary-marquee-${title.replace(/\s+/g, "-").toLowerCase()}`}
          >
...
function TestimonialsMarquee({ className = "" }: { className?: string }) {
  ...
  return (
    <Marquee className={className} paused={paused} pauseOnHover={false}>
